...
class Upload extends Common {
    ...
    public function img(){
        ...
        $info = $file->validate(['size'=>2097152,'ext'=>'jpg,jpeg,png,gif,bmp'])->rule('date')->move(ROOT_PATH."public".$path,'');
        if(empty($info)){
    ...
    public function file(){
        ...
        $info = $file->rule('date')->move(ROOT_PATH."public".$path,'');
        if(empty($info)){
            $this->result["msg"]=$file->getError();
        }else{
            $this->result["code"]=0;
