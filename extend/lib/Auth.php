...
class Auth{
    ...
    public function getGroups($uid) {
        ...
        $user_groups =db($this->auth_group_access)->alias('a')
            ->where($where)
            ->join($this->auth_group." g "," a.group_id=g.id")
    ...
    public function getRules($rules,$order=[]){
        ...
        $user_groups = db($this->auth_rule)->where($map)->order($order)->select();
    ...
    public function getId($name){
        $authRuleId = db($this->auth_rule)->where('name',$name)->field('parent_id')->find();
        if(empty($authRuleId)){
            return 0;
        }else{
    ...
    public function getFirstId($id){
        $authRuleFirstId = db($this->auth_rule)->where('id',$id)->field('parent_id')->find();
    ...
    protected function getAuthList($uid,$type) {
        ...
        //读取用户组所有权限规则
        $rules = db($this->auth_rule)->where($map)->field('condition,name')->select();
        //循环规则，判断结果。
        $authList = [];   //
    ...
    protected function getUserInfo($uid) {
        ...
        if(!isset($userInfo[$uid])){
            $userInfo[$uid]=db($this->auth_user)->where('id',$uid)->find();
        }
        return $userInfo[$uid];
    }
}